...
from yad2k.model.keras_yolo import yolo_boxes_to_corners
...
def yolo_filter_boxes(box_confidence, boxes, box_class_probs, threshold=.6):
    ...
    # Step 3: Create a filtering mask based on "box_class_scores" by using "threshold". The mask should have the same dimension as box_class_scores, ans be True for the boxes with probabilty >= Threshold

    filtering_mask = box_class_probs >= threshold
...
def iou(box1, box2):
    """Implementat the intersection over the union {IoU} between box1 and box2

    Arguments:
    box1 -- First box, list object woth coordinates (x1, y1, x2, y2)
...
def yolo_non_max_suppression(scores, boxes, classes, max_boxes=10, iou_threshold=0.5):
    ...
    k.get_session().run(tf.initializers([max_boxes_tensor]))

    # This function will return indices of selected boxes
    nms_indices = tf.image.non_max_suppression(
        boxes, scores, max_boxes_tensor, iou_threshold)

    # Selected only those scores that are in nms_indices
...
def yolo_eval(yolo_output, image_shape=(720., 1280.), max_boxes=10, score_threshold=.6, iou_threshold=.5):
    """
    ...
    # Retrival output of the YOLO model
    box_confidence, box_xy, box_wh, box_class_probs = yolo_output

    # Retrival boxes to be ready for filtering functions

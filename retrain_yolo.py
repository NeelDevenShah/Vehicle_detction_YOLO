...
def _main(args):
    ...
    draw(model_body,
         ...
         anchors,
         image_data,
         image_set='val',  # assume training/validation split is 0.9
...
def create_model(anchors, class_names, load_pretrained=True, freeze_body=True):
    ...
    final_layer = Conv2D(len(anchors)*(5+len(class_names)),
                         (1, 1), activation='linear')(topless_yolo.output)

    model_body = Model(image_input, final_layer)

    # Place mode loss on CPU to reduce GPU memory uasge.
...
def train(model, class_names, anchors, image_data, boxes, detectors_mask, matching_true_boxes, validation_split=0.1):
    ...
    early_stopping = EarlyStopping(
        monitor='val_loss', min_delta=0, patience=15, verbose=1, model='auto')
    ...
    model_body, model = create_model(
        anchors, class_names, load_pretrained=False, freeze_body=False)
    ...
    )  # This is for the usage of the custom loss function in the last layer.
    ...
    model.save_weights('trained_stage_3_best.h5')
